...
public class AudibleClient extends WebClient {
    ...
    /*
     * String DEFAULT_MOBILE_USER_AGENT_STRING =
     ...
     * ;
     */
    // want a consistent user agent.. but not sure how much it makes a difference,
    // if any.
    public static String swtWindows = "Mozilla/5.0 (Windows NT 10.0; Win64; x64; Trident/7.0; rv:11.0) like Gecko";

    public AudibleClient() {
        super(new BrowserVersion.BrowserVersionBuilder(BrowserVersion.FIREFOX_ESR).setUserAgent(swtWindows).build());
        ...
        if (useJS) {
            JavaScriptEngine scriptEngine = (JavaScriptEngine) getJavaScriptEngine();
